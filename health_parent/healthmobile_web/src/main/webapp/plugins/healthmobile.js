...
function getUrlParam(paraName) {
    //http://localhost:83/pages/setmeal_detail.html?id=12
    var url = document.location.toString();
    //alert(url);
    //0 = [http://localhost/pages/setmeal_detail.html]
    //1= [id=12&u=34]
    var arrObj = url.split("?");//根据？好分割
    if (arrObj.length > 1) {
        var arrPara = arrObj[1].split("&");// [id=12] [u=34]
        var arr;
        for (var i = 0; i < arrPara.length; i++) {
            arr = arrPara[i].split("=");//[id] [12]
